//! DOM-like tree taken from
//! [here](https://github.com/SimonSapin/rust-forest/blob/master/idtree/lib.rs)
...
impl NodeId {
    pub fn level<T>(&self, arena: &Tree<T>) -> usize {
        let mut level = 0;
        for _ in self.ancestors(arena) {
            level += 1;
        }
        level-1
    }
    ...
    pub fn ancestors<T>(self, arena: &Tree<T>) -> Ancestors<T> {
        Ancestors {
            arena: arena,
            node: Some(self),
        }
    ...
    ///
    /// Call `.next().unwrap()` once on the iterator to skip the node itself.
    pub fn preceding_siblings<T>(self, arena: &Tree<T>) -> PrecedingSiblings<T> {
        PrecedingSiblings {
            arena: arena,
            node: Some(self),
        }
    ...
    ///
    /// Call `.next().unwrap()` once on the iterator to skip the node itself.
    pub fn following_siblings<T>(self, arena: &Tree<T>) -> FollowingSiblings<T> {
        FollowingSiblings {
            arena: arena,
            node: Some(self),
        }
    ...
    pub fn children<T>(self, arena: &Tree<T>) -> Children<T> {
        Children {
            arena: arena,
    ...
    pub fn reverse_children<T>(self, arena: &Tree<T>) -> ReverseChildren<T> {
        ReverseChildren {
            arena: arena,
    ...
    /// Call `.next().unwrap()` once on the iterator to skip the node itself.
    pub fn descendants<T>(self, arena: &Tree<T>) -> Descendants<T> {
        Descendants(self.traverse(arena))
    }

    /// Return an iterator of references to this node and its descendants, in tree order.
    pub fn traverse<T>(self, arena: &Tree<T>) -> Traverse<T> {
        Traverse {
            arena: arena,
            root: self,
    ...
    /// Return an iterator of references to this node and its descendants, in tree order.
    pub fn reverse_traverse<T>(self, arena: &Tree<T>) -> ReverseTraverse<T> {
        ReverseTraverse {
            arena: arena,
            root: self,
